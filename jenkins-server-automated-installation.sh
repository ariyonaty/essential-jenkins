...
echo "# $(date -Imin) Installation is starting."
...
echo "# $(date -Imin) Instatll jenkins key and package configuration..."
...
echo "# $(date -Imin) Install Java 11, NGINX, and Jenkins..."
apt update
apt-get -qq upgrade
...
# configure jenkins
echo "# $(date -Imin) Configure Jenkins..."

## skip the installation wizard at startup
echo "# $(date -Imin) Skip the installation wizard on first boot..."
echo "JAVA_ARGS=\"-Djenkins.install.runSetupWizard=false\"" >> /etc/default/jenkins

## download the list of plugins
echo "# $(date -Imin) Download the list of plugins..."
...
echo "# $(date -Imin) Use the keyword 'suggest' to find the suggested plugins in the list..."
grep suggest platform-plugins.json | cut -d\" -f 4 | tee suggested-plugins.txt

## download the plugin installation tool
echo "# $(date -Imin) Download the plugin installation tool"
...
echo "# $(date -Imin) Run the plugin installation tool..."
...
echo "# $(date -Imin) Update the permissions on the plugins directory..."
chown -R jenkins:jenkins /var/lib/jenkins/plugins

# configure nginx
echo "# $(date -Imin) Configure NGINX..."
...
echo "# $(date -Imin) Reload NGINX to pick up the new configuration..."
systemctl reload nginx

# install docker
echo "# $(date -Imin) Install docker..."
...
echo "# $(date -Imin) Restart Jenkins..."
systemctl restart jenkins

echo "# $(date -Imin) Copy the initial admin password to the root user's home directory..."
